...
Design doc for supporting non-Linear ticketing systems in advisory projects. Not yet prioritized.

**Ticket:** [PRJ-3](https://linear.app/health-tool/issue/PRJ-3/support-non-linear-ticketing-systems-for-advisory-projects)

## Current State

All ticketing integration assumes Linear:

- **`registry.yaml`** stores `linear.team_id` and `linear.team_key` per personal project. Advisory projects (e.g., Booster) have `tools.ticketing: null` as a placeholder.
...
- **Plan filenames** use Linear team_key prefixes (e.g., `HT-42-plan.md`).
- **`settings.json` template** enables `linear@claude-plugins-official` for all projects by default.
- No other skill (`/aggregate`, `/propagate`, `/update-project`) queries ticketing APIs directly — they read local docs only.
...
# Advisory project using Jira
booster:
  ticketing:
    system: jira
    project_key: BOOST
    base_url: https://booster.atlassian.net

# Advisory project using GitHub Issues
some-project:
  ticketing:
...
Personal projects keep `system: linear`. The `tools.ticketing` field on advisory projects becomes unnecessary since `ticketing` is a first-class block.
...
`/propagate` and `/update-project` would customize the plugins list based on the project's ticketing system:
...
Currently `HT-42-plan.md` uses Linear team_key. Options:
- Keep ticket-number prefixes but use the system's own format (e.g., `BOOST-42` for Jira, `#42` or `issue-42` for GitHub Issues)
- Let each project's CLAUDE.md specify the convention
- This is cosmetic and low priority

### MCP server config (`.mcp.json`)

Advisory projects need MCP servers matching their tool stack. The metaproject could scaffold this:
- Linear project: linear MCP server config
- Jira project: Jira MCP server config
- This is already contemplated in Use Case 5 (advisory team setup)
...
- `settings.json` plugin list per project type
- `.mcp.json` scaffolding per project's tools

**Out of scope:**
- Building MCP servers for Jira/Asana (these are third-party)
- Migrating personal projects away from Linear
- Automated ticket creation/updates from skills (no skill does this today)

## When to Build

When an advisory project needs a non-Linear ticketing system. Booster's ticketing system is still TBD — once it's known, that's the trigger to implement this if it's not Linear.
